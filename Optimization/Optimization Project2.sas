...
data returns;
	set master;
	STXr = dif(STX)/lag(STX)*100;
	AMDr = dif(AMD)/lag(AMD)*100;
	ITr = dif(IT)/lag(IT)*100;
	NOWr = dif(NOW)/lag(NOW)*100;
	FTVr = dif(FTV)/lag(FTV)*100;
...
proc optmodel;
	...
	*con RisklessReturn: 0.015 <= sum{i in Assets1}Proportion[i]*Mean[i] + (1 - sum{i in Assets1}Proportion[i])*0.005;
	...
	/* Print Solutions */
	print Covariance Mean 'Return ='(sum{i in Assets1}Proportion[i]*Mean[i]);
...
/*Use OPTMODEL to setup and solve the minimum variance problem*/ 
  ...
  set parameter_values = {0.001 to 0.4 by 0.005}; 
...
data export;
	set obj_value_Stddev_results;
	keep Portfolio_Stdev_Results Expected_Return_Results;
run;

proc export data=export outfile="C:\Users\samir\Documents\Courses\Optimization\ProjectData\frontierdata.xlsx" replace dbms=xlsx;
run;
